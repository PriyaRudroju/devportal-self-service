...
| Invalid Git branch in Port form | Branch does not exist on GitHub | Port workflow **Validate Git Branch (Lambda)** fails at `POST /s3/validate-git-ref` with HTTP 400; entity stays **`pending`**; automation and GitHub do not run |
| Validate step 404 | Lambda route or `API_GATEWAY_URL` wrong | Deploy integration Terraform (`POST /s3/validate-git-ref`); use base URL only in `API_GATEWAY_URL` |
| GitHub branch check fails in Lambda | Missing or invalid `GITHUB_TOKEN` on Lambda | Set `github_token` in TFC workspace `dev-portal-integration-dev` and re-apply integration stack |
